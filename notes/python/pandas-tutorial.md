...
> 配套笔记：接单时的速查手册见 [pandas-core-operations.md](./pandas-core-operations.md)（能力地图 + 场景模板）
>
> **学习原则**：本章目标是"看懂 pandas 代码在干什么"。语法糖细节（为什么 `df['列']` 能这样写）不懂没关系，先跑起来，用到再查。
...
注意：返回的不是普通 list，而是带索引的 `Series`。想要普通列表就 `col.tolist()`。
...
> `agg(新列名=('源列', '统计方式'))` 这种写法第一次看到会觉得怪，不用深究语法细节，记住这个模板就能套。
...
**你已经掌握的**（看懂 + 能改）：
- 读 CSV / Excel、看数据、筛选、排序、加列、分组统计、清洗、导出
- 看懂第 10 章那种完整脚本的每一步

**下一步**：
1. 把本文代码全部跑一遍，改改参数看结果变化
2. 用你爬虫项目爬到的 `books.csv` 做练习：读入 → 筛选 → 分组统计 → 导出 Excel
3. 看 [pandas-core-operations.md](./pandas-core-operations.md)——那里面有 7 个接单场景模板（合并多文件、多表关联、拆分表格等），用到哪个套哪个

**遇到不会的**：直接问 AI，提问时说清楚"输入数据长什么样 + 想要什么样 + 现在卡在哪"。
